...
public:
    DeviceInfo()
            : InfoClassType(Device_None), Type(Device_None), Version(0) {
...
protected:
    DeviceInfo(DeviceType type)
            : InfoClassType(type), Type(type), Version(0) {
        ProductName[0] = Manufacturer[0] = 0;
    }
...
public:
    ...
    bool MatchRule(DeviceType type, bool available) const {
        return ((EnumType == type) || (EnumType == Device_All)) && (available || !AvailableOnly);
    }
...
public:
    ...
    T* CreateDevice() {
        return static_cast<T*>(DeviceHandle::CreateDevice());
    }
...
public:
    ...
    virtual DeviceManager* GetManager() const {
        return const_cast<DeviceManager*>(this);
    }
...
public:
    ...
    HMDInfo()
            : DeviceInfo(Device_HMD), HResolution(0), VResolution(0), HScreenSize(0), VScreenSize(0), VScreenCenter(0), EyeToScreenDistance(
                    0), LensSeparationDistance(0), InterpupillaryDistance(0), DesktopX(0), DesktopY(0), DisplayId(0) {
    ...
    bool IsSameDisplay(const HMDInfo& o) const {
        return DisplayId == o.DisplayId && String::CompareNoCase(DisplayDeviceName, o.DisplayDeviceName) == 0;
    }
...
struct SensorRange {
    SensorRange(float maxAcceleration = 0.0f, float maxRotationRate = 0.0f, float maxMagneticField = 0.0f)
            : MaxAcceleration(maxAcceleration), MaxRotationRate(maxRotationRate), MaxMagneticField(maxMagneticField) {
    }
...
public:
    SensorInfo()
            : DeviceInfo(Device_Sensor), VendorId(0), ProductId(0) {
        SerialNumber[0] = 0;
    }
...
public:
    ...
    // Static constant for this device type, used in template cast type checks.
    enum {
        EnumDeviceType = Device_LatencyTester
    };

    virtual DeviceType GetType() const {
        return Device_LatencyTester;
    }
