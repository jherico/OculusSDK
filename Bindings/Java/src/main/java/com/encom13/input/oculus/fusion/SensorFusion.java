package com.encom13.input.oculus.fusion;

import com.encom13.input.oculus.tracker.TrackerMessage;
import com.encom13.input.oculus.tracker.TrackerSample;
import com.encom13.input.oculus.tracker.TrackerVector;
import com.google.common.base.Predicate;

import copied.com.jme3.math.Matrix4f;
...
public class SensorFusion
{
    ...
    private final Quaternion   QUncorrected          = new Quaternion();

    private final Vector3f     RawAccel              = new Vector3f();
    private final Vector3f     AngV                  = new Vector3f();
    private float              AngVL                 = 0;
    private final Vector3f     CalMag                = new Vector3f();
    private final Vector3f     RawMag                = new Vector3f();
    ...
    private Vector3f           TiltErrorAxis         = new Vector3f(Vector3f.UNIT_Y);
    ...
    public void processSensorMessage() {
        ...
        if (AngVL > 0.0f) {
            Q.multLocal(new Quaternion(AngVL * DeltaT, AngV.divide(AngVL), true));
        }
    ...
    private void tiltCorrect() {
        ...
        if ((Math.abs(accLength - 9.81f) < GRAVITY_EPSILON) && (AngVL < GYRO_EPSILON))
            TiltCondCount++;
        else
            TiltCondCount = 0;
        ...
        if (TiltCondCount >= TILT_PERIOD) {
            ...
            if (tiltAngle > MAX_TILT_ERROR)
            {
                ...
                TiltErrorAxis = new Vector3f(accWMean.z, 0.0f, -accWMean.x).normalizeLocal();
            }
        }
        ...
        if (TiltErrorAngle > MIN_TILT_ERROR) {
            if ((TiltErrorAngle > 0.4f) && (RunningTime < 8.0f)) {
                ...
                Q.multLocal(new Quaternion(-TiltErrorAngle, TiltErrorAxis));
                TiltErrorAngle = 0.0f;
            } else {
                // This uses aggressive correction steps while your head is
                // moving fast
                float deltaTiltAngle = -Gain * TiltErrorAngle * 0.005f * (5.0f * AngVL + 1.0f);
                // Incrementally "un-tilt" by a small step size
                Q.multLocal(new Quaternion(deltaTiltAngle, TiltErrorAxis));
    ...
    public synchronized Quaternion getPredictedOrientation(float pdt) {
        Quaternion qP = new Quaternion(QUncorrected);
        if (predictionEnabled) {
            ...
            Vector3f angVelF = new Vector3f(AngV);
            ...
            if (angVelFL > 0.001f) {
                qP.multLocal(new Quaternion(angVelFL * newpdt, angVelF.divide(angVelFL), true));
            }
        }
        return qP;
    }
    ...
    public class MagCalibration {
        final Matrix4f     MagCalibrationMatrix    = new Matrix4f();
        long               MagCalibrationTime      = -1;                               ;
        boolean            MagCalibrated           = false;
        int                MagCondCount            = 0;
        float              MagRefDistance          = 0.5f;
        final Quaternion   MagRefQ                 = new Quaternion();
        final Vector3f     MagRefM                 = new Vector3f(Vector3f.ZERO);
        float              MagRefYaw               = 0;
        ...
        {
            Vector3f mag = new Vector3f(rawMag);
            // mag.x += MagCalibrationMatrix.M[0][3];
            // mag.y += MagCalibrationMatrix.M[1][3];
            // mag.z += MagCalibrationMatrix.M[2][3];
            return mag;
        ...
        void yawCorrect() {
            ...
            if (MagCalibrated && (RunningTime > 2.0f) && (MagCondCount >= magWindow) && MagHasNearbyReference) {
