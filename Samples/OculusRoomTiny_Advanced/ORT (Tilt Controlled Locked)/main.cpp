...
/// Press '1' to disable the effect, although not advised, and a
...
int WINAPI WinMain(HINSTANCE hinst, HINSTANCE, LPSTR, int)
{
    ...
    while (basicVR.HandleMessages())
    {
        ...
        basicVR.MainCam->Rot = Matrix4f();

        for (int i = 0; i < 2; i++)
        {
            ovrQuatf storedOrientation;
            if (!DIRECTX.Key['1'])
            {
                storedOrientation = basicVR.Layer[0]->EyeRenderPose[i].Orientation;
                float proportionOfNormal = 0.5f;// 50:50 of normal tilt/roll/yaw, and only yaw;
                float rotx, roty, rotz;
                ((Quatf)basicVR.Layer[0]->EyeRenderPose[i].Orientation).GetEulerAngles<Axis_X, Axis_Y, Axis_Z>(&rotx, &roty, &rotz);
                Quatf baseOne = Quatf(Matrix4f().RotationY(roty));
                basicVR.Layer[0]->EyeRenderPose[i].Orientation = ((Quatf)basicVR.Layer[0]->EyeRenderPose[i].Orientation).Lerp(baseOne, 1 - proportionOfNormal);
            }
            basicVR.Layer[0]->RenderSceneToEyeBuffer(basicVR.MainCam, basicVR.pRoomScene, i);

            if (!DIRECTX.Key['1'])
            {
                // Now put it back
                basicVR.Layer[0]->EyeRenderPose[i].Orientation = storedOrientation;
            }
        }
