...
bool HSWDisplay::Initialize(const ovrRenderAPIConfig* apiConfig)
{
    ...
    if(config)
    {
        ...
        RenderParams.ContextData = config->OGL.ContextData;
        RenderParams.ContextSwitch = config->OGL.ContextSwitch;
        RenderParams.SwapBuffers = config->OGL.SwapBuffers;
    }
    else
    {
        UnloadGraphics();
...
void HSWDisplay::UnloadGraphics()
{
    ...
    {
        RenderParams.ContextSwitch(RenderParams.ContextData, true);
        ...
        RenderParams.ContextSwitch(RenderParams.ContextData, false);
    }
}
