...
public:
    ...
    bool AddBitsAndReallocate( const BitSize_t numberOfBitsToWrite );
    ...
    template <>
    bool ReadCompressed(String &var);
...
private:
    ...
    BitStream( const BitStream & /*invalid*/) : numberOfBitsUsed(0), numberOfBitsAllocated(0), readOffset(0),data(nullptr), copyData(false) {
        OVR_ASSERT(0);
    }
...
/// \param[in] var The value to write
template <>
inline void BitStream::Write(const String &inTemplateVar)
{
...
template <>
inline bool BitStream::Read(String &outTemplateVar)
{
    bool b;
    uint16_t l;
    b=Read(l);
    if (b && l>0)
    {
        ...
        if ((int)GetNumberOfUnreadBits() < (int)l * 8)
        {
            OVR_ASSERT(false); // Corrupted data
            return false;
        }
...
#if 0
// These are not safe to use.  We need to specify the maximum length of the string buffer
