...
void BitStream::Write( const char* inputByteArray, const unsigned int numberOfBytes )
{
    ...
    BitSize_t numberOfBits = BYTES_TO_BITS(numberOfBytes);
    if (BITS_TO_BYTES(numberOfBits) != numberOfBytes)
        return; // overflow
    if (numberOfBits + numberOfBitsUsed < numberOfBits)
        return; // overflow

    // Optimization:
    if ((numberOfBitsUsed & 7) == 0)
    {
        if (AddBitsAndReallocate(numberOfBits))
        {
...
void BitStream::Write( BitStream* bitStream, BitSize_t numberOfBits )
{
    ...
    if (AddBitsAndReallocate(numberOfBits) == false)
        return;
...
{
    if (numberOfBitsUsed + 1 == 0)
        return; // overflow

    if (AddBitsAndReallocate(1) == false)
        return;
...
//        return;
    ...
    if (numberOfBitsUsed + numberOfBitsToWrite < numberOfBitsUsed)
        return; // overflow

    if (AddBitsAndReallocate(numberOfBitsToWrite) == false)
        return;
...
bool BitStream::AddBitsAndReallocate( const BitSize_t numberOfBitsToWrite )
{
    BitSize_t newNumberOfBitsAllocated = numberOfBitsToWrite + numberOfBitsUsed;
    if (newNumberOfBitsAllocated < numberOfBitsUsed)
        return false;
