...
static bool SetSocketOptions(SocketHandle sock)
{
    ...
    sock_opt = 1;
    result |= setsockopt(sock, IPPROTO_TCP, TCP_NODELAY, (char *)& sock_opt, sizeof(sock_opt));

    // For Windows 8+ use the fast loopback path.
    int OptionValue = 1;
    DWORD NumberOfBytesReturned = 0;
    WSAIoctl(sock, SIO_LOOPBACK_FAST_PATH, &OptionValue, sizeof(OptionValue), nullptr, 0, &NumberOfBytesReturned, 0, 0);
...
{
    OVR_ASSERT(bufferCount < 16);
    
    if (bufferCount >= 16)
        return -1;
    if (buffers == NULL || buffersLengths == NULL)
        return -1;

    WSABUF wsaBuffers[16];
    for (int i = 0; i < bufferCount; i++)
    {
        OVR_ASSERT(buffersLengths[i] > 0);
        wsaBuffers[i].len = buffersLengths[i];
        wsaBuffers[i].buf = (CHAR *) buffers[i];
    }

    DWORD numberOfBytesSent = 0;
    WSASend(TheSocket, wsaBuffers, bufferCount, &numberOfBytesSent, 0, NULL, NULL);
